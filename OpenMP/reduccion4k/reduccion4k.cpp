...
int main(int argc, char** argv) {
    ...
    if (THREADS== 2)
    {
        fprintf(pFile, "Time elapsed transforming a 4k image to 480p using OpenMP with 16 threads: %ld.%06lds\n", (long int)tval_result.tv_sec, (long int)tval_result.tv_usec);    
        fclose(pFile);
    }
    else if (THREADS == 8)
    {
        fprintf(pFile, "Time elapsed transforming a 4k image to 480p using OpenMP with 4 threads: %ld.%06lds\n", (long int)tval_result.tv_sec, (long int)tval_result.tv_usec);    
        fclose(pFile);
    }
    else if (THREADS == 4)
    {
        fprintf(pFile, "Time elapsed transforming a 4k image to 480p using OpenMP with 8 threads: %ld.%06lds\n", (long int)tval_result.tv_sec, (long int)tval_result.tv_usec);    
        fclose(pFile);
    }
    else if (THREADS == 16)
    {
        fprintf(pFile, "Time elapsed transforming a 4k image to 480p using OpenMP with 2 threads: %ld.%06lds\n", (long int)tval_result.tv_sec, (long int)tval_result.tv_usec);    
        fclose(pFile);
    }
    else
    {
