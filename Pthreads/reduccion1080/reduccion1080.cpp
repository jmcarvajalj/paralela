...
void* transform1080to480(void* arg);

struct transform1080to480_struct{
    Mat image;
    Mat result;
};
...
int main(int argc, char** argv) {
    ...
    if (argc < 4) {
        ...
        cerr << "Uso:" << argv[0] << " Imagen-Entrada Imagen-Salida #Hilos (Ejemplo:./reduccion 1080.jpg result.jpg 8)"<< endl;
    ...
    //Create Attributes
    struct transform1080to480_struct args[num_threads];

    for(int i=0; i<num_threads;i++){    
    args[i].image = imread(argv[1], IMREAD_COLOR);    
    pthread_attr_t attr;
    pthread_attr_init(&attr);
    pthread_create(&tids[i],&attr, transform1080to480, &args[i]);
    }
    ...
    if (num_threads== 2)
    {
        fprintf(pFile, "Time elapsed transforming a 1080p image to 480p using PThreads with 16 threads: %ld.%06lds\n", (long int)tval_result.tv_sec, (long int)tval_result.tv_usec);    
        fclose(pFile);
    }
    else if (num_threads == 8)
    {
        fprintf(pFile, "Time elapsed transforming a 1080p image to 480p using PThreads with 4 threads: %ld.%06lds\n", (long int)tval_result.tv_sec, (long int)tval_result.tv_usec);    
        fclose(pFile);
    }
    else if (num_threads == 4)
    {
        fprintf(pFile, "Time elapsed transforming a 1080p image to 480p using PThreads with 8 threads: %ld.%06lds\n", (long int)tval_result.tv_sec, (long int)tval_result.tv_usec);    
        fclose(pFile);
    }
    else if (num_threads == 16)
    {
        fprintf(pFile, "Time elapsed transforming a 1080p image to 480p using PThreads with 2 threads: %ld.%06lds\n", (long int)tval_result.tv_sec, (long int)tval_result.tv_usec);    
        fclose(pFile);
    }
    else
    {
        fprintf(pFile, "Time elapsed transforming a 1080p image to 480p using PThreads with %d threads: %ld.%06lds\n", num_threads, (long int)tval_result.tv_sec, (long int)tval_result.tv_usec);    
        fclose(pFile);
    }    
    return 0;
}
...
void* transform1080to480(void* arg){
    ...
    struct transform1080to480_struct *arg_struct = (struct transform1080to480_struct*) arg;
    ...
    Mat copy( (arg_struct->image.rows*4)/9, arg_struct->image.cols/3, CV_8UC3, Scalar(255,255, 255));
