...
int main(int argc, char** argv) {
    ...
    if (num_threads== 2)
    {
        fprintf(pFile, "Time elapsed transforming a 720p image to 480p using PThreads with 16 threads: %ld.%06lds\n", (long int)tval_result.tv_sec, (long int)tval_result.tv_usec);    
        fclose(pFile);
    }
    else if (num_threads == 8)
    {
        fprintf(pFile, "Time elapsed transforming a 720p image to 480p using PThreads with 4 threads: %ld.%06lds\n", (long int)tval_result.tv_sec, (long int)tval_result.tv_usec);    
        fclose(pFile);
    }
    else if (num_threads == 4)
    {
        fprintf(pFile, "Time elapsed transforming a 720p image to 480p using PThreads with 8 threads: %ld.%06lds\n", (long int)tval_result.tv_sec, (long int)tval_result.tv_usec);    
        fclose(pFile);
    }
    else if (num_threads == 16)
    {
        fprintf(pFile, "Time elapsed transforming a 720p image to 480p using PThreads with 2 threads: %ld.%06lds\n", (long int)tval_result.tv_sec, (long int)tval_result.tv_usec);    
        fclose(pFile);
    }
    else
    {
